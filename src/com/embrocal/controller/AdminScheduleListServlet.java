...
@WebServlet("/admininterviewsched.html")
public class AdminScheduleListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("admin")!=null) {
			try {
				if (connection != null) {
					ResultSet rs = SQLOperations.getAllApplicantEntrySchedule(
							connection, "UNAVAILABLE");
					request.setAttribute("schedule", rs);
					
						getServletContext().getRequestDispatcher(
								"/admininterviewsched.jsp").forward(request,
								response);
